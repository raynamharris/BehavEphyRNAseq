...
```{r removingsamples}
# removing outliers RNAseq samples
countbygene <- countbygene %>%
  filter(RNAseqID != "146C-CA3-4", RNAseqID != "16-116D") 
tpmbygene <- tpmtbygene %>%
  filter(RNAseqID != "146C-CA3-4", RNAseqID != "16-116D") 
tpm <- tpm %>%
  filter(RNAseqID != "146C-CA3-4", RNAseqID != "16-116D") 
count <- count %>%
...
# remove samples that don't have rnaseq data 
savesamples <- colnames(countbygene) # list of all samples with kallisto data
kallistosamples <- tidysamples %>% filter(RNAseqID %in% savesamples) %>% droplevels() # removes samples without kallisto data
