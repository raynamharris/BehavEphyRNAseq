...
## Loading the data
In the summers of 2015 and 2016, I processed a bunch of hippocampal tissue samples from WT and FMR1-KO. Most mice were trained in an active place avoidance task or used as yoked controls; however, a few animals were taken straight from the home cage.  

This data was added the the epically large sample collection database contained in the two files  "animals.csv" and "punches.csv" which provided a detailed account of all animals processed and all tissue samples collected. Then, I tidy the dataframe a little bit to get it prepared for RNAseq sample submision. I didn't always have the forsight to get the data in the propoer column for subsequent anlaysis, but some data wrangly magic will make it tidy.
